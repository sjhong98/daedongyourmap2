...
export type CountType = {point: string, count: number}

export const GetCount = async (state: string, email?: string) => {
    ...
    for(let i=0; i<16; i++) {
        ...
        let getQuery;
        if(state === "main") getQuery = query(posts, where("point", "==", locations[i].point));
        else getQuery = query(posts, where("point", "==", locations[i].point), where("user", "==", email));
